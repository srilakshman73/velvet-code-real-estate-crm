      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="px-3 py-1 text-xs font-semibold bg-[#547A61]/10 text-[#547A61] border border-[#547A61]/20 rounded-full">
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-[#FFF9F0] border border-[#D8C7A5] shadow-[0_8px_24px_rgba(120,90,40,0.08)] space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-2">
          ...
          <div className="space-y-2 p-4 rounded-xl bg-[#F4EAD7] border border-[#D8C7A5] shadow-2xs">
            ...
            <p className="text-[10px] text-[#6A5A44]">Enforced on backend</p>
          </div>

          {/* 2. Leads */}
          <div className="space-y-2 p-4 rounded-xl bg-[#F4EAD7] border border-[#D8C7A5] shadow-2xs">
            <div className="flex justify-between text-xs font-semibold">
              <span className="text-[#6A5A44]">Lead Storage:</span>
              <span className="text-[#2C241A] font-mono">
            ...
            <p className="text-[10px] text-[#6A5A44]">Active buyer records</p>
          </div>
          ...
          <div className="space-y-2 p-4 rounded-xl bg-[#F4EAD7] border border-[#D8C7A5] shadow-2xs">
            ...
            <div className="w-full h-2 bg-[#E9DFC8] rounded-full overflow-hidden border border-[#D8C7A5]">
              <div
                className="h-full bg-[#C39A5B] rounded-full"
                style={{
                  width: `${Math.min(
                    (properties.length /
            ...
            <p className="text-[10px] text-[#6A5A44]">Live active inventory</p>
          </div>
          ...
          <div className="space-y-2 p-4 rounded-xl bg-[#F4EAD7] border border-[#D8C7A5] shadow-2xs">
            ...
            <p className="text-[10px] text-[#6A5A44]">Resets on next cycle</p>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-[#D8C7A5]/60">
              {INITIAL_INVOICES.map((inv) => (
                <tr key={inv.id} className="hover:bg-[#F7EEDC] transition-colors">
                  <td className="p-4 font-bold text-[#2C241A] font-mono">{inv.invoiceNumber}</td>
                  <td className="p-4 text-[#6A5A44]">Monthly SaaS Subscription</td>
                  <td className="p-4 font-extrabold text-[#7A5520] font-mono">
                    ₹{inv.totalINR}
                  </td>
                  <td className="p-4 text-[#6A5A44]">{inv.paymentMethod}</td>
                  <td className="p-4">
                    <span className="px-2.5 py-0.5 text-xs font-bold bg-[#547A61]/10 text-[#547A61] rounded border border-[#547A61]/20">
                      Paid
                    </span>
                  </td>
                  <td className="p-4 text-right">
                    <button
                      ...
                      className="text-[#7A5520] hover:underline font-semibold flex items-center gap-1 ml-auto text-xs cursor-pointer"
                    >
            ...
            return (
              <div
                ...
                className={`p-5 rounded-2xl border flex flex-col justify-between space-y-4 transition-all ${
                  isCurrent
                    ? 'bg-[#A37432]/10 border-[#A37432] shadow-md'
                    : 'bg-[#FFF9F0] border-[#D8C7A5]'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {isCurrent && (
                      <span className="px-2 py-0.5 text-[9px] font-bold bg-[#A37432] text-white rounded">
                        Current
                      </span>
                    )}
                  </div>
                  <p className="text-2xl font-serif font-extrabold text-[#2C241A] mt-2">
